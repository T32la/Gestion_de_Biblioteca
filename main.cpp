...
//    cin.ignore();
    ...
    cout << endl << endl;  
		cout<< h << h << "Los datos del libro han sido registrados correctamente en el sistema..."<<endl;
   		system("pause");
   		system("cls");
}
        ...
        if (op_buscar == "titulo") {
            ...
            for (int i = 0; i<libros.size(); i++) {
                if (libros[i].titulo == buscar) {			   
	        	       cout << h << libros[i].titulo << h << libros[i].autor << h 
                   << libros[i].genero << h << libros[i].anio_publicacion << h 
                   << libros[i].estatus << endl;
                   }
            } 
            ...
            for (int i =0; i<libros.size(); i++) {
                if (libros[i].genero == buscar) {
                    cout << h << libros[i].titulo << h << libros[i].autor 
                    << h << libros[i].genero << h << libros[i].anio_publicacion 
                    << h << libros[i].estatus << endl;  
				        } 
		        }
        } else {
...
void actualizarInformacion(vector<Libro>& libros) {
    ...
    while (true) {
        cout << h 
        << "¿Actualizar informacion del libro por titulo o estatus? (Escribe: titulo o estatus): ";
        cin >> op_buscar;
        cin.ignore();
        ...
        if (op_buscar == "titulo") {
            ...
            for (auto& libro : libros) {
                if (libro.titulo == buscar) {
                    ...
                    cout << h << "Titulo: " << libro.titulo << h 
                         << "Autor: " << libro.autor << h<< "Genero: " 
                         << libro.genero << h << "Año: " << libro.anio_publicacion  
                         << h << "Estatus: " << libro.estatus << endl;
            ...
            for (auto& libro : libros) {
                if (libro.titulo == buscar) {
                    ...
                    << libro.genero 
					          << h << ", Año: " 
                    << libro.anio_publicacion << h << ", Estatus: " 
                    << libro.estatus << endl;
